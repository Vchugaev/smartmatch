...
@Injectable()
export class AdminProfileStrategy implements ProfileStrategy {
  constructor(private prisma: PrismaService) {}

  async createProfile(createAdminProfileDto: any, userId: string) {
    ...
    if (user?.role !== 'ADMIN') {
      throw new ForbiddenException('Only ADMIN users can create admin profiles');
    }
  ...
  async deleteProfile(userId: string) {
    ...
    return this.prisma.adminProfile.delete({
      where: { userId },
    });
  }
